...
def calculate_production(
    ...
    try:
        ...
        new_table_name = 'economic_calculation_output_' + uuid.uuid4().hex[:4]
        ...
        message = (
            f"Production calculation successful. The calculated production data are stored in the database table: '{new_table_name}'.\n"
...
def calculate_combined_royalty_rate(
    userID: Annotated[str, InjectedState("userID")],
    table_name: Annotated[str, "The table containing the production data from which royalty will be calculated"],
    c_star: float,
...
def calculate_revenue(
        userID: Annotated[str, InjectedState("userID")],
        table_name: Annotated[str, "The table containing the production data from which revenue will be calculated"],
        oil_production_col: Annotated[str, "The column name for oil production in the table"],
        gas_production_col: Annotated[str, "The column name for gas production in the table"],
        propane_production_col: Annotated[str, "The column name for propane production in the table"],
        butane_production_col: Annotated[str, "The column name for butane production in the table"],
        oil_price: Annotated[float, "The price of oil in $/bbl"] = 0.0,
        gas_price: Annotated[float, "The price of gas in $/mcf"] = 0.0,
        propane_price: Annotated[float, "The price of propane in $/bbl"] = 0.0,
        butane_price: Annotated[float, "The price of butane in $/bbl"] = 0.0
    ) -> Dict:
    """
    ...
    Parameters:
    - userID: The unique identifier for the user accessing the data.
    - table_name: The name of the table containing production data.
    - oil_production_col: The column name for monthly oil production (in bbl).
    - gas_production_col: The column name for monthly gas production (in mcf).
    - propane_production_col: The column name for monthly propane production (in bbl).
    - butane_production_col: The column name for monthly butane production (in bbl).
    - oil_price: The price of oil ($/bbl).
    - gas_price: The price of gas ($/mcf).
    - propane_price: The price of propane ($/bbl).
    - butane_price: The price of butane ($/bbl).
...
def calculate_monthly_costs(
        userID: Annotated[str, InjectedState("userID")],
        table_name: Annotated[str, "The correct table containing the boe value"],
        boe_col: Annotated[str, "The column name for BOE data in the table"],
    ...
    try:
        ...
        # Extract BOE data
        if boe_col not in df.columns:
            raise ValueError(f"Column '{boe_col}' does not exist in the table '{table_name}'. Use calculate_production tool to calculate.")

        boe = df[boe_col].values
        periods = len(boe)
...
def calculate_monthly_cashflow(
        userID: Annotated[str, InjectedState("userID")],
        table_name: Annotated[str, "The correct table containing the BOE value"],
        monthly_total_opex_col: Annotated[str, "Column name for monthly total operating expenses"],
        monthly_transportation_col: Annotated[str, "Column name for monthly transportation costs"],
        monthly_capex_col: Annotated[str, "Column name for monthly capital expenditures"],
    ...
    try:
        ...
        # Extract data
        monthly_total_opex = df[monthly_total_opex_col].values
        monthly_transportation = df[monthly_transportation_col].values
        monthly_capex = df[monthly_capex_col].values

        # Extract optional data
        monthly_oil_revenue = extract_or_default(df, monthly_oil_revenue_col)
        monthly_gas_revenue = extract_or_default(df, monthly_gas_revenue_col)
        monthly_propane_revenue = extract_or_default(df, monthly_propane_revenue_col)
        monthly_butane_revenue = extract_or_default(df, monthly_butane_revenue_col)
        oil_royalty_rate = extract_or_default(df, oil_royalty_rate_col)
        gas_royalty_rate = extract_or_default(df, gas_royalty_rate_col)
        propane_royalty_rate = extract_or_default(df, propane_royalty_rate_col)
        butane_royalty_rate = extract_or_default(df, butane_royalty_rate_col)
...
def calculate_metrics(
    userID: Annotated[str, InjectedState("userID")],
    table_name: Annotated[str, "The name of the table containing the cash flow data"],
    monthly_free_cash_flows_col: Annotated[str, "The column name for the monthly free cash flow"],
    discount_rate: Annotated[float, "Annual discount rate (default 10%)" ]= 0.1
) -> Dict:
    """
    ...
    try:
        ...
        # Extract the monthly free cash flows column
        if monthly_free_cash_flows_col not in df.columns:
            return {"status": "error", "message": f"Column '{monthly_free_cash_flows_col}' not found in the table '{table_name}'."}
        
        monthly_free_cash_flows = df[monthly_free_cash_flows_col].values
