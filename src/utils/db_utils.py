...
sqlite3.register_adapter(list, list_in)
sqlite3.register_converter("list", list_out)
CONVERTER_FUNCTINOS["list_out"] = list_out
...
def store_data_sqlite3(filename=":memory:", table=None, data=None, type=None, description=None):
    ...
    filename = os.path.join("database", filename + ".db")
...
def db_check(db_name):
    ...
    Args:
        db_name (str): The name of the database file (without .db extension).

    Returns:
        None
    """
    db_path = os.path.join("database", db_name + ".db")

    # Check if the database file exists in the current directory
    if not os.path.isfile(db_path):
        # If it doesn't exist, create an empty database
...
def table_check(db_name, table_name, columns={"data": "BLOB", "type": "TEXT", "description": "TEXT"}):
    ...
    db_name = os.path.join("database", db_name + ".db")
...
#################################################
def get_db_json(db_name='test.db', table_name="test", columns_to_exclude=["data", ], output_type='string'):
...
# All Table Schema
#################################################
def get_all_db_json(db_name='test.db', excluded_tables=["running"], rows_limit=3):
    """
    Retrieve the schema and first several rows of all tables in the SQLite database, except for 
    the excluded tables.

    Args:
        db_name (str): The name of the SQLite database file.
        excluded_tables (list): A list of table names to exclude from the result.
        rows_limit (int): The number of rows to retrieve from each table.

    Returns:
        str: JSON formatted schema and data for all tables except the excluded ones.
    """
    # Connect to the SQLite database
    db_name = os.path.join("database", db_name + ".db")
    ...
    # Initialize a dictionary to store schema and data
    db_info = {}
    ...
    for table in tables:
        ...
        # Construct the SQL query to fetch the first few rows
        query = f"SELECT * FROM {table} LIMIT {rows_limit}"
        cursor.execute(query)
        rows = cursor.fetchall()

        # Store the schema and data in the dictionary
        db_info[table] = {
            "schema": columns,
            "data": [dict(zip(columns, row)) for row in rows]
        }

    # Convert the db_info dictionary to JSON format
    json_results = json.dumps(db_info)
